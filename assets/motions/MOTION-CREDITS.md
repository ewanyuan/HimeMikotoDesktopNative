...
本目录中的 VMD 只作为本地桌面宠物的动作素材保存。每段动作都保留了原作者随包提供的说明或条款文件；如果以后公开发布程序或录制视频，应再次核对对应条款并完整保留署名。
...
| 东京・香迪・兰德弗（风格化） | `tokyo-shandy-rendezvous/P_TokyoSR_モーション配布/TokyoSR_motion.vmd` | [P-nez 动作页](https://p-nez.net/tokyo_m)；动作：P-nez | 原作者说明从配布视频音源起始位置开始使用；本程序作为无音乐动作播放。 |
| QUEEN（酷帅） | `queen/QUEEN_miku.vmd` | [P-nez 动作索引](https://p-nez.net/motions_)；动作：P-nez | 选用动作包内的 Miku 适配版本；本程序作为无音乐动作播放。 |
| Womanizer（双人・魅惑） | `womanizer/womanizer-left.vmd` + `womanizer/womanizer-right.vmd` | [BowlRoll 34830](https://bowlroll.net/up/dl34830)；动作：MON | 原包将左、右两个舞者拆成两份 VMD；桌宠菜单会临时同时加载 Hime 与 Mikoto，分别播放两套动作。原包说明允许改编，但禁止再分发和商业使用。 |
...
| つよっ！（单人） | `requested/tsuyoi/tsuyoi.vmd` | [BowlRoll 345944](https://bowlroll.net/file/345944)；动作：イースアリシア；乐曲：NEKOZUME feat. 最上もが | 随包要求署名；禁止再分发、商业使用和 R18。 |
| Tick-Trick（Rick式） | `requested/tick-trick/Tick-Trick.vmd` | [BowlRoll 308751](https://bowlroll.net/file/308751)；动作：Rick；乐曲作者：前線 | 条款禁止 R15 以上、暴露度高的模型和商业使用，并禁止再分发。 |

## 素材库保留、暂不接入菜单

| 动作 | VMD 目录 | 暂不接入原因 |
| --- | --- | --- |
| はなまるセカイ | `hanamaru-sekai/P_はなまるセカイ_モーション/` | 原包是双人舞台编排；在单人物桌宠画布上会产生明显横向舞台位移和侧卧/裁切姿态。已确认 VMD 能加载，但目前画面质量不达标，所以不让用户选到。 |
| INMU KING | — | 原视频页面只提供视频/音乐相关素材，没有公开且可核验的 VMD 动作文件；不手工编造近似动作。 |
| Oppai Fukkireta | — | 原作者的公开条款禁止将动作数据嵌入游戏；在没有单独授权前不集成。 |
| 爱♡スクリ〜ム！ | — | 用户指定来源：`ss46752642`；[BowlRoll 358490](https://bowlroll.net/file/358490) | 对应动作文件尚未取得，因此不进入菜单。 |

## 条款摘要

- P-nez 的[使用条款](https://p-nez.net/terms-of-use)禁止利用数据获得金钱利益、将数据本身或使用数据再次分发/销售，以及隐藏版权表示；公开使用时还要求标示动作、音乐、编舞参考和角色等相应版权信息。
...
- `爱♡スクリ〜ム！` 的动作文件仍待从原页面取得；在取得前不把其它舞蹈误标为该动作，也不在菜单显示占位项。
